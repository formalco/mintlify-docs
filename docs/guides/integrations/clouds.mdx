...
<Accordion title="ECS Autodiscovery" icon="cubes">
  Discover ECS clusters, services, and containers for SSH access via the Desktop
  App.
</Accordion>
...
<Accordion title="Resources not discovered">
  **Possible causes:** - Autodiscovery not enabled for that resource type -
  ...
  1. Verify autodiscovery toggle is enabled in Formal console 2. Check resources
  exist in AWS 3. Review CloudFormation IAM role permissions 4. Wait a few
  minutes for initial scan
</Accordion>
