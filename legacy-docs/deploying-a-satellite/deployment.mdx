...
Deploy Formal Satellites with the right environment
...
- `FORMAL_CONTROL_PLANE_API_KEY`: This variable represents the satellite's API Key issued by Formal in order for the Satellite to authenticate itself with the Control Plane.
  ...
  <img src="/assets/images/formal_control_plane_api_key.png" />

The different Satellites also expose Satellite-specific environment variables that you can find below.

<Warning>
  Ensure that you replace `Your_Formal_Satellite_API_Key_Here`,
  `Your_Private_Key_Content_Here` and `Your_Satellite_Certificate_Content_Here`
  with the actual content of the respective certificates and keys in the
...
<CardInfo>
  The Policy Data Loader Satellite passes its environment to its workers,
  meaning that you can use environment variables in the code of your Policy Data
  Loaders if you deploy the Satellite with those.
</CardInfo>
